...
const PROTECTED_APP = "control-panel";
...
export async function middleware(request: NextRequest) {
  ...
  if (secret) {
    ...
    if (payload.exp <= nowSeconds) {
      const refreshed = await attemptRefresh(payload);
...
async function attemptRefresh(payload: SessionPayload): Promise<SessionPayload | null> {
